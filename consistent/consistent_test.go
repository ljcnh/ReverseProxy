...
func TestAdd(t *testing.T) {
	...
	if len(c.sorted) != replicationFactor {
...
func TestRemove(t *testing.T) {
	...
	if len(c.sorted) != 0 && len(c.hosts) != 0 {
...
func TestIncDone(t *testing.T) {
	...
	c.Inc(host)
	fmt.Println(c.hostMap[host].Load)
	if c.hostMap[host].Load != 1 {
		t.Fatalf("host %s load should be 1\n", host)
	}

	c.Done(host)
	if c.hostMap[host].Load != 0 {
...
func TestHosts(t *testing.T) {
	...
	fmt.Println("hosts in the ring", c.GetHosts())

	addedHosts := c.GetHosts()
	for _, h := range hosts {
		found := false
...
func TestDelSlice(t *testing.T) {
	...
	c := &Consistent{}
	c.sorted = append(c.sorted, items...)
